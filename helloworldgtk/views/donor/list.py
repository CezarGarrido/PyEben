...
class DonorListForm(BaseTab):
    ...
    def create_content(self):
        ...
        self.add_column("Nome", 1)
        self.add_status_column("Status", 2)
        self.add_column("CPF/CNPJ", 3)
        self.add_column("Criado em", 4)
        self.add_column("Atualizado em", 5)
    ...
    def load_users(self):
        ...
        users = service.list(self.app.logged_user)
        self.store.clear()
        ...
        if users:
            for user in users:
                self.store.append((
                    ...
                    user.name,
                    user.active,
                    user.cpf if user.person_type != "PJ" else user.cnpj,  # Papel (ID ou nome)
    ...
    def on_new_clicked(self, action):
        ...
        form.connect("donor_added", lambda _, user_id: self.load_users())
        form.show_all()
        GLib.idle_add(form.update_fields_visibility)
    ...
    def on_edit_user_clicked(self, button):
        ...
        if tree_iter:
            ...
            edit_form.connect("donor_updated", lambda _, user_id: self.load_users())
            edit_form.show_all()
            GLib.idle_add(edit_form.update_fields_visibility)
        else:
            self.show_error_dialog("Nenhum usuário selecionado.")
